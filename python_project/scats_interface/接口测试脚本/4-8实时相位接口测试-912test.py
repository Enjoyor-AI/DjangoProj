...
def request_task1(url, payload2, num):
    ...
    try:
        ...
        if Content.DATASAMPLE:
            GetRealTimePhase = Content.SampleDataAll
        else:
        ...
        if Content.DATASAMPLE:
            ...
            try:
                for i in range(len(GetRealTimePhase)):
                    print(i)
                    list_RealTimePhase = runRealtimePhase(GetRealTimePhase[i], list_RealTimePhase)
                    result = result + list_RealTimePhase
                    print(len(result))
            except Exception as e:
                print(e)
...
def create_process():
    ...
    # print(int_list)
    print("All Int NUM:", len(int_list))
...
def phase_order_send(order_result):
    ...
    for data in order_result:
        ...
        for phase in phase_order.keys():
            ...
            send_data.append([site_id, phase, phase_length, str(order_num), create_time, cycle_time])
    print("相序数据收集完毕")
    print(send_data)
                ...
                if siteid is not None:
                    if siteid not in keys:
                        ...
                        if order_result[siteid]['RequireCycle'] != None:
                            ...
                            # if phase_name not in phase_interval[siteid].keys():
                            #     # phase_interval[siteid][phase_name] = i[3] + i[9]
                            #     order_result[siteid]['PhaseOrder'][phase_name] = i[3] + i[9]
                            # else:
                            #     if order_result[siteid]['RequireCycle'] != require_cycle and\
                            #             len(order_result[siteid]['PhaseOrder']) > 0:
                            #
                            #         phase_order_send(order_result)
                            #         sql =Content.sql_phase_order
                            #
                            #     order_result[siteid]['PhaseOrder'] = []
                            # if phase_name not in order_result[siteid]:
...
def call_phase_data():
    pg = Postgresql()
    conn,cr = pg.db_conn()
    sql = "select * from st_realtime_phase_inf_912back where {0} > '2018-09-10 00:00:00' and {0} < '2018-09-10 20:00:00' order by {0}".format("\"PUSHTIME\"")
    cr.execute(sql)
    rows = cr.fetchall()
    pg.db_close()
    return rows
...
if __name__ == '__main__':
    ...
    data = call_phase_data()
    print(data)
    phase_inf_resolve(data)
    print(result)
