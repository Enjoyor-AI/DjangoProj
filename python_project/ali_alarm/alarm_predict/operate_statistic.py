...
sql_operate_static = " SELECT userid,time_serice,to_timestamp(operdate||' '||to_char(time_serice*{0}/60,'99')||':'||" \
                     ...
                     "as operdate from record_data_resolve where opertype is not null and  opertype != '' " \
...
sql_distinct = "select distinct userid, opertime,string_agg(opertype,',')as all_type,siteid," \
      ...
      "to_char(opertime,'yyyy-mm-dd')as operdate from record_data_resolve " \
...
pg_inf_145 = {'database': "signal_specialist", 'user': "postgres", 'password': "postgres",
...
class OperateStatistics():
    ...
    def alarm_operate_match(self):
        self.pg = Postgres()
        conn,cr = self.pg.dbconn()

        if cr:
            cr.execute(sql_alarm_operate_match.format('33.83.100.145', '2018-10-19 00:00:00',
                                                      '2018-10-19 23:59:59', LIMIT_TIME ))
            conn.commit()
