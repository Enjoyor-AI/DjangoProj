    ...
    try:
        ...
        try:
            if len(int_list) > 1:
                ...
                sql = "select sys_code from pe_tobj_node_info  where node_id = '{}' ".format(int_list[0])
            # cr = conn.cursor()
...
def call_oracle2(logger, month):
    exist_maintain = pd.DataFrame({})
    try:
        ora = Oracle()
        if month:
            sql = "select distinct fstr_name, fstr_id from MAINTAIN_LIST where substr(fstr_date, 1, 7) = '{0}'".format(month)
            # print(sql)
        else:
...
def call_oracle4(cur_date_str, listid, logger):
    ...
    try:
        ...
        #       "select fstr_intersectid, fstr_date from MAINTAIN_LIST where fstr_id = '{0}' ) a " \
        #       "left join hz_scats_detector_state b on a.fstr_intersectid = b.fstr_intersectid " \
        #       "where b.FSTR_DATE = '2018-11-12' AND b.FSTR_ERRORCODE " \
        #       "in ('ERR0001', 'ERR0002', 'ERR0003', 'ERR0004', 'ERR0005')".format(listid)
...
def main_get_maintain_list(logger, month=None):
    exist_maintain = call_oracle2(logger, month)
