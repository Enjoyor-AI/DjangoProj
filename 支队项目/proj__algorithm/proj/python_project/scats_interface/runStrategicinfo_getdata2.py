...
def CallOracle():
    ...
    match_records = []
    try:  # 数据库连接超时即退出程序
        db = cx_Oracle.connect(CONSTANT.OracleUser, threaded=True)
        cr = db.cursor()
                    ...
                    if CountLaneNo == 0:
                        ...
                        if CONSTANT.IF_CHECK:
                            if [str(SiteID), PhaseBitMask] not in resolve_fail_node:
                                ...
                                lock.acquire()
                                try:
                                    f = open('E:\PROGRAME\log\scats_strategic_resolve_fail_node' + str(date) + '.txt',
                                             'r+')
                                    f.read()
...
def RequestDynaDataFromInt(siteIDlist, IntStrInput,n):
	...
	"resultList": [{
		...
		"Id": 598036211,
		"IsSALK": true,
		"NominalCycleTime": 118,
		"RecvTime": "2018-12-10 00:02:00",
		"Region": "XH2",
		"RequiredCycleTime": 102,
		"SalkList": [{
			"ADS": 23,
			"DS1": 0,
			"DS2": null,
		...
		"Id": 598037755,
		"IsSALK": true,
		"NominalCycleTime": 109,
		"RecvTime": "2018-12-10 00:04:00",
		"Region": "XH2",
		"RequiredCycleTime": 102,
		"SalkList": [{
			"ADS": 11,
		...
		"Id": 598039235,
		"IsSALK": true,
		"NominalCycleTime": 111,
		"RecvTime": "2018-12-10 00:05:00",
		"Region": "XH2",
		"RequiredCycleTime": 97,
		"SalkList": [{
			"ADS": 11,
			"DS1": 25,
			"DS2": null,
...
def MainProcess():
    ...
    if len(IntersectInfo) > 0:
        ...
        try:
            conn = cx_Oracle.connect(CONSTANT.OracleUser, threaded=True)  # 连接数据库
        except Exception as e:
