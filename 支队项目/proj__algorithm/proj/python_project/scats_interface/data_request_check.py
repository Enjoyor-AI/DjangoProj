...
from .runStrategicinfo_getdata2 import int_grouped, RequestDynaDataFromInt
from .Request_Data_From_Int import RequestDynaDataFromInt as operate_request
from proj.tools.func_timer import timer
import threading
from proj.config.database import Postgres, ConnectInf
...
class InterfaceCheck():
    ...
    def scats_id_init(self):
        ...
        try:
            conn = cx_Oracle.connect(SqlText.OracleUser)  # 连接数据库
        except Exception as e:
            print('MainProcess:连接数据库失败', e)
        else:
            cr = conn.cursor()  # 建立游标
    ...
    @timer
    def salk_list_request(self, date):
        ...
        if date == 'yesterday':
            ...
            today = (dt.datetime.now()).strftime('%Y-%m-%d') + ' 00:00:00'
            tomorrow = (dt.datetime.now() + dt.timedelta(days=1)).strftime('%Y-%m-%d') + ' 00:00:00'
            loss_time = self.loss_data_period_check('战略运行记录接口', today, tomorrow)
        print('loss_time:%s' % loss_time)
        # thread_creat(self.int_group, self.scats_id_list, self.int_str_input)
        # loss_time = ['2018-12-13 16:50:00']
    ...
    def loss_data_period_check(self, interface_name, stime, etime):
        ...
        if result is not None and not result.empty:
            ...
            dev = np.std(data_num)
            print("%s-%s~%s各时段平均请求量:%s;标准差：%s" % (interface_name, stime,etime,avg_num, dev))
            if avg_num > 0:
    ...
    def operate_request(self,date):
        ...
        if date == 'yesterday':
            ...
            stime = (dt.datetime.now()).strftime('%Y-%m-%d') + ' 00:00:00'
            etime = (dt.datetime.now()).strftime('%Y-%m-%d') + ' 23:59:59'
...
def main(date='yesterday'):
    I = InterfaceCheck()
    I.salk_list_request(date)
    I.operate_request(date)
