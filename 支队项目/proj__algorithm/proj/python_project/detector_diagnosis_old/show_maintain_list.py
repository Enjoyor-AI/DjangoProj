...
def call_oracle(cur_date_str, scatsid, logger):
    ...
    try:
        ...
        sql = "select a.FINT_DETECTORID, a.FSTR_ERRORCODE, a.FSTR_ERRORNAME, a.FSTR_ERRORDETAIL, b.PHASENO from(" \
        ...
        try:
            ...
            available_score = ora.call_oracle_data(sql)
            # cr = db.cursor()
            # cr.execute(sql1)
            # rs1 = cr.fetchall()
    ...
    return detector_detail, available_score
...
def main_detector_detail(cur_date_str, scatsid, logger):
    ...
    detector_detail, available_score = call_oracle(cur_date_str, scatsid, logger)
    print(laneInfor)
    ...
    if not detector_detail.empty:
        ...
        merge_detector_detail.rename(columns={'FINT_DETECTORID': 'detectorId', 'FSTR_ERRORNAME': 'dataType',
                                              'FSTR_ERRORDETAIL': 'dataDetail', 'PHASENO': 'phase'}, inplace=True)
        print(merge_detector_detail)
