...
import datetime
#from proj.python_project.scats_operate_parsing_wr import operate_record_resolve
import operate_record_resolve
import cx_Oracle
import threading
from datetime import timedelta, date, datetime

# from database import pg_inf,OracleUser
#from proj.python_project.scats_operate_parsing_wr import state_information_feedback
import state_information_feedback
from proj.config import database as database_gai
#from proj.python_project.scats_operate_parsing_wr import content
import content
...
def task():
    global net_state
    try:
        timer = threading.Timer(6000, task)
        timer.start()
    ...
    orcl = database.Oracle()
    starttime = '2018-11-29 00:00:00'
    ...
    try:
        ...
        print("get operate data success!")
        if operate_data:
            '''后续操作'''
            try:
                ...
                print("当前时间为：%s 操作完成，下次将在1分钟后进行" % nowTime)
        else:
            print('匹配不到操作记录解析数据')
...
def main():
    ...
    cycle_init = pg.call_pg_data(sql=content.CONTENT.sql_init_cycle)
    cycle_dit = {}
    if cycle_init:
        for i in cycle_init:
            cycle_dit[i[0]] = i[1]
    ...
    if int_split:
        for split in int_split:
            scats_id = split[0]
            cycle = cycle_dit.get(scats_id)
            int_list.append(scats_id)
            split_locked = split[1]
            scats_split_init[scats_id] = {'updatatime': None, 'user_name': None, 'spilt': None, 'cycle': cycle,
                                          'coordination': None,
