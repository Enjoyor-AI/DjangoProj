...
from proj.config import database
from proj.python_project.scats_operate_parsing_wrong import seperate_operate_record
#from proj.python_project.scats_operate_parsing_wr import database_gai
from proj.config import database as database_gai
#from proj.python_project.scats_operate_parsing_wr import content
import content
from PlanSelection import main
...
def insert(operate_int_states):
    pp = {'database': "signal_specialist", 'user': "postgres", 'password': "postgres",
    ...
    for key in operate_int_states.keys():
        ...
        if value:
            ...
            sql_plan = "select gaode_id from gaode_inter_scatsid where scats_id='{0}'".format(siteid)
            cr.execute(sql_plan)
            gaode_id = cr.fetchall()
            if gaode_id == []:
                gaode_id = ''
            try:
                plan_number = main(gaode_id)
            except Exception as e:
                print('the planselection error is:', e)
                plan_number = 'unknow'

            list = [updatetime, user, spilt, cycle, coordination, pp, dwell, xsf, other, siteid, plan_number]
            sql = content.CONTENT.sql10
            cr.execute(sql, list)
            db.commit()
    cr.close()
    db.close()
