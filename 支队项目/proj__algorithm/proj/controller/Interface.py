...
from ..controller import TaskModel
import json
from ..config.database import Postgres,Oracle,ConnectInf
from ..config.sql_text import SqlText
        ...
        if 'scatsID' in request.GET:
            ...
            if scatsid == 'all':
                ...
                try:
                    ...
                    sql1 = "select sys_code, node_id from pe_tobj_node_info"
                    IntInfor = pg.call_pg_data(sql1, fram=True)
                    IntInfor.rename(columns={'sys_code': 'FSTR_INTERSECTID'}, inplace=True)
                    # print(IntInfor)
                    pg.db_close()

                    O1 = Oracle()
                    result_df = O1.call_oracle_data(sql, fram=False)
                    result_df = pd.merge(result_df, IntInfor, on='FSTR_INTERSECTID', how='left')
                    # print(result_df)
                ...
                if read_pkl:
                    file_dir = os.path.dirname(os.getcwd()) + \
                               '\\proj\\proj\\python_project\\detector_diagnosis\\sched_task\\detector_available.pkl'
                    print('getAvailability file dir', file_dir)
                    result_df = pd.read_pickle(file_dir)
                    temp_df = result_df[result_df.FSTR_INTERSECTID == scatsid]
                    if not temp_df.empty:
                        result_json = {'scatsID': temp_df.iloc[0]['FSTR_INTERSECTID'],
                                       'score': temp_df.iloc[0]['SCORE']}
                    else:
                        result_json = {}
                    # print(result_json)
...
def getDetectorList(request):
    ...
    try:
        ...
        print('getDetectorList')
        file_dir = os.path.dirname(os.getcwd()) + \
                   '\\proj\\proj\\python_project\\detector_diagnosis\\sched_task\\detector_list.pkl'
        print('getDetectorList file dir', file_dir)
        # merge_detector_all = pd.read_csv(file_dir, dtype={'scatsId': str, 'score': str, 'laneNum': str})
        merge_detector_all = pd.read_pickle(file_dir)
            ...
            if 'PageSize' in request.GET and 'PageNum' in request.GET:
                ...
                if not merge_detector_all.empty:
                    ...
                    if not result_df.empty:
                        ...
                        result = {'appCode': 1, 'result': {'dataLength': len(merge_detector_all), 'data': result_df}}
                    else:
                        result = {'appCode': 0, 'result': {'dataLength': 0, 'data': []}}
            if 'nodeId' in request.GET:
                nodeid = request.GET.get('nodeId')
                result_df = merge_detector_all[merge_detector_all.nodeId == nodeid]
                if not result_df.empty:
                    result_df = result_df.to_json(orient='records', force_ascii=False)
                    result_df = json.loads(result_df)
                    result = {'appCode': 1, 'result': {'dataLength': 1, 'data': result_df}}
                else:
                    result = {'appCode': 0, 'result': {'dataLength': 0, 'data': []}}
        else:
            if not merge_detector_all.empty:
                result_df = merge_detector_all.to_json(orient='records', force_ascii=False)
                result_df = json.loads(result_df)
                result = {'appCode': 1, 'result': {'dataLength': len(merge_detector_all), 'data': result_df}}
            else:
                result = {'appCode': 0, 'result': {'dataLength': 0, 'data': []}}
...
# 连inter_info的接口有getDetectorList, getDetectorDetail, getDetectorPlot, addMaintainList, 还有detector_task
    ...
    if request.GET:
        if 'nodeId' in request.GET:
            ...
            try:
                ...
                print(cur_date_str, 'getDetectorDetail')
                merge_detector_detail, available_score = main_detector_detail(cur_date_str, scatsid, log)
                # print('merge_detector_detail', merge_detector_detail)
...
def getDetectorPlot(request):
    ...
    if request.GET:
        if 'nodeId' in request.GET and 'detectorId' in request.GET:
            ...
            cur_date_str = dt.datetime.strftime(cur_date_dt, "%Y-%m-%d")
            # cur_date_str = '2018-11-12'
            print(cur_date_str, 'getDetectorPlot')

            try:
                file_dir = os.path.dirname(os.getcwd()) + \
                           '\\proj\\proj\\python_project\\detector_diagnosis\\sched_task\\detector_plot.pkl'
                print('getDetectorPlot file dir', file_dir)
                fr = open(file_dir, 'rb')
                inf = pickle.load(fr)
                fr.close()
                try:
                    n = json.loads(inf)
                    data = n[scatsid][detectorid]
                    plot_data = {'flow': data['flow'], 'ds': data['ds'], 'time': data['time']}
                    detectorCode = data['detectorid']
                    detectorState = data['state']
                    result = {'appCode': 1, 'result': {'detectorCode': detectorCode,
                                                       'detectorState': detectorState, 'plotData': plot_data}}
                except KeyError:
...
def addMaintainList(request):
    if request.method == 'POST':
        try:
            ...
            cur_date_dt = dt.datetime.now()
            cur_date_str = dt.datetime.strftime(cur_date_dt, "%Y-%m-%d")
            # cur_date_str = '2018-11-12'
            if json_result['listNode']:
...
def getMaintainList(request):
    ...
    print('getMaintainList')
    try:
        if request.GET and 'month' in request.GET:
            month = request.GET.get('month')
            exist_maintain = main_get_maintain_list(log, month=month)
        else:
...
def getAbnormalIntersect(request):
    ...
    try:
        ...
        file_dir = os.path.dirname(os.getcwd()) + \
                   '\\proj\\proj\\python_project\\detector_diagnosis\\sched_task\\abnormal_int.pkl'
        print('getAbnormalIntersect file dir', file_dir)
        # f = open(os.path.dirname(os.getcwd()) + '\\proj\\proj\\python_project\\detector_diagnosis\\abnormal_int.pkl')
        # x = f.read()
        # f.close()
        ...
        log.error(e)
        print('getAbnormalIntersect ' + str(e))
...
#获取配置信息
def get_configure_info(request): #接口形式
    pp=Postgres()
    dict={}
    if request.method=='GET':      #http://192.168.25.61:8088/get_configure_info?starttime=2018-12-10&endtime=2018-12-14
        if 'starttime' in request.GET and 'endtime' in request.GET:
            ...
            sql="select * from scats_configure_update_log where time between '{0}' and '{1}' order by time".format(starttime,endtime)
            data=pp.call_pg_data(sql,pg_inf=demo_pg_inf)
            response1 = JsonResponse(data, safe=False, json_dumps_params={'ensure_ascii': False})
            return response1