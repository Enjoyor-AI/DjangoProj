...
class CONSTANT:
    ...
    S_REPAIR_DATE = '2018-12-17 12:00:00'
    E_REPAIR_DATE = '2018-12-20 09:00:00'
...
def SkitListOut(runstrinformation, row, list_AnalyzedRunInfo):
    ...
    for i in RunStrInfo:      # 遍历resultList中的数据元素
        ...
        R_cycle_time = datetime.strptime(RecvTime, '%Y-%m-%d %H:%M:%S')
                ...
                if CountLaneNo == 0:
                    for n in range(4):
                        ...
                        if DS is not None and VO is not None:
                            itemlist = [SiteID, SALKNo, SALKNo * 100 + n+1, FstrCycleStartTime, OutPhaseTime,
                                        PhaseBitMask, PhaseTime, ActualCycleTime, DS, VO, DATE_FstrDate,
                                        WeekDay, ConfigVersion]
                            list_AnalyzedRunInfo.append(itemlist)
...
def RequestDynaDataFromInt(siteIDlist, IntStrInput,n):
    ...
    for m in range(len(siteIDlist)):
        ...
        # 4.3获取战略运行记录
        try:
            RunStrInformation = requests.get(r'http://33.83.100.138:8081/getStrategicmonitor.html',
                                             params=payload1,timeout=10)  # 4.3
    ...
    if len(list_AnalyzedRunInfo) > 0:
        ...
        print('thread:', n, 'scats接口数据接收完毕,数据量:%s'%len(list_AnalyzedRunInfo))
    elif len(list_AnalyzedRunInfo) == 0:
        Log.info('thread:%s no salklist data received!' % n )
        print("scats接口获取数据失败")
    # lock.acquire()
    # ORACLE链接threaded=True无需加线程锁
    try:
...
def thread_creat(group_num, int_grouped,IntStrInput):
    ...
    try:
        ...
        if not CONSTANT.IF_DATA_REPAIR:
            ...
            date_range = pd.date_range(S_REPAIR_DATE,E_REPAIR_DATE, freq='0.5H').tolist()
            print(date_range)
...
def scats_salk_data_send(list_RunStrInfo, list_RunStrInfoSalkList, list_AnalyzedRunInfo, n):
    ...
    try:
        conn = cx_Oracle.connect(CONSTANT.OracleUser,threaded=True)
    except Exception as e:
        ...
        try:
            if len(list_RunStrInfo) > 0:
                ...
                Log.info('thread: %s list_RunStrInfo insert successfully!,data num: %s' % (n,len(list_RunStrInfo)))
            print("thread:%s list_RunStrInfo写入完毕,数据量%s" % (n,len(list_RunStrInfo)))
            if len(list_RunStrInfoSalkList) > 0:
                WriteDynaData(conn, list_RunStrInfoSalkList, CONSTANT.sql_RunStrInfoSalkList)
                Log.info('thread:%s list_RunStrInfoSalkList insert successfully!,data num: %s' % (n,len(list_RunStrInfoSalkList)))
            print("thread:%s list_RunStrInfoSalkList写入完毕,数据量%s" % (n,len(list_RunStrInfoSalkList)))
            if len(list_AnalyzedRunInfo) > 0:
                WriteDynaData(conn, list_AnalyzedRunInfo, CONSTANT.sql_AnalyzedRunInfo)  # 解析结果
                Log.info('thread: %s list_AnalyzedRunInfo insert successfully!,data num: %s'% (n,len(list_AnalyzedRunInfo)))
            print("thread:%s list_AnalyzedRunInfo写入完毕,数据量%s" % (n,len(list_AnalyzedRunInfo)))
        except Exception as e:
            pass
        finally:
            conn.commit()
...
def django_test():
    ...
    try:
        ...
        print('django_test2', e)
    else:
        print('success')
        print(group)
        thread_creat(int(group), int_list, scats_input)
