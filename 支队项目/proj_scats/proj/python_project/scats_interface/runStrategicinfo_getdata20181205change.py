...
class CONSTANT:
    ...
    S_REPAIR_DATE = '2018-11-22 01:00:00'
    E_REPAIR_DATE = '2018-11-23 17:00:00'
    ...
    try:
        ...
        try:
            if len(list_RunStrInfo) > 0:
                ...
                Log.info('thread: %s list_RunStrInfo insert successfully!' % n)
            print("thread:%s list_RunStrInfo写入完毕" % n)
            if len(list_RunStrInfoSalkList) > 0:
                WriteDynaData(conn, list_RunStrInfoSalkList, CONSTANT.sql_RunStrInfoSalkList)
                Log.info('thread:%s list_RunStrInfoSalkList insert successfully!' % n)
            print("thread:%s list_RunStrInfoSalkList写入完毕" % n)
            if len(list_AnalyzedRunInfo) > 0:
                WriteDynaData(conn, list_AnalyzedRunInfo, CONSTANT.sql_AnalyzedRunInfo)  # 解析结果
                Log.info('thread: %s list_AnalyzedRunInfo insert successfully!'% n)
            print("thread:%s list_AnalyzedRunInfo写入完毕" % n)
        except Exception as e:
            pass
        finally:
            conn.commit()
